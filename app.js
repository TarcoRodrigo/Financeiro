...
function mkTxItem(t){
  ...
  if(t.tipo==='despesa'&&!t.cartaoId){
    ...
    if(pg){var at=dataD(pg)>dataD(t.data);bdg.style.cssText='background:rgba(255,113,113,.12);color:var(--red);border-radius:4px;padding:1px 5px;font-size:9px;font-weight:600;margin-left:4px;';bdg.textContent=at?'Atraso '+fData(pg):'Pago '+fData(pg);}
...
function rInicio(el){
  ...
  topo.style.cssText='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;';
  var topoL=document.createElement('div');
  var saldoEl=document.createElement('div');
  saldoEl.style.cssText='font-size:34px;font-weight:300;color:var(--text);letter-spacing:-2px;line-height:1;';
  ...
  topo.appendChild(topoL);topo.appendChild(topoR);
  el.appendChild(topo);

  // GRID entradas / saidas
  var grid=document.createElement('div');
  grid.style.cssText='display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:22px;';
  function mkMetrica(lbl,val,cor){
    var c=document.createElement('div');c.className='metrica';
    var l=document.createElement('div');l.className='metrica-label';l.textContent=lbl;
    var v=document.createElement('div');v.className='metrica-val';v.style.color=cor;v.textContent=privado?'R$ ****':fR(val);
  ...
  if(fpendTotal>0){
    ...
    apR.appendChild(apV);apR.appendChild(apL);apHdr.appendChild(apT);apHdr.appendChild(apR);
    ...
    listaAP.slice(0,5).forEach(function(item,idx){
      ...
      var dataStr=diff<0?'Venceu':diff===0?'Hoje':'Vence '+fData(item.tipo==='fat'?item.f.dataVenc:item.t.data);
      var row=document.createElement('div');
      row.style.cssText='display:flex;align-items:center;padding:12px 14px;gap:10px;'+(idx>0?'border-top:.5px solid var(--border2);':'');
      ...
      else{btn.addEventListener('click',(function(tid){return function(e){e.stopPropagation();pagarHoje(tid);};})(item.t.id));}
      right.appendChild(vEl);right.appendChild(btn);
      row.appendChild(ic);row.appendChild(info);row.appendChild(right);
  ...
  if(depPend>0){
    ...
    proj.style.cssText='margin-top:12px;padding:12px 14px;background:var(--bg2);border-radius:14px;display:flex;justify-content:space-between;align-items:center;';
    var pL=document.createElement('span');pL.style.cssText='font-size:12px;color:var(--text3);';pL.textContent='Projetado (pagando tudo)';
    var pV=document.createElement('span');pV.style.cssText='font-size:13px;font-weight:600;color:'+(saldoProj>=0?'var(--accent)':'var(--red)')+';';pV.textContent=fR(saldoProj);
    proj.appendChild(pL);proj.appendChild(pV);el.appendChild(proj);
  }

  // CONTAS (resumo compacto)
  if(d.contas.length>0){
    ...
    var cL=document.createElement('span');cL.className='sec-link';cL.textContent='+ Nova';cL.addEventListener('click',function(){abM('sh-conta');});
    chdr.appendChild(cT);chdr.appendChild(cL);el.appendChild(chdr);
    var hs=document.createElement('div');hs.className='hscroll';
    d.contas.forEach(function(c){var b=banco(c.banco);var cd=document.createElement('div');cd.className='ac-card';cd.innerHTML='<div class="ac-bar" style="background:'+b.cor+'"></div><div class="ac-banco">'+b.sigla+' '+(c.nome||b.nome)+'</div><div class="ac-saldo">'+fRs(c.saldo||0)+'</div><div class="ac-tipo">'+c.tipo+'</div>';hs.appendChild(cd);});
    var cadd=document.createElement('div');cadd.className='ac-card add';cadd.textContent='+ Conta';cadd.addEventListener('click',function(){abM('sh-conta');});hs.appendChild(cadd);
    el.appendChild(hs);
  }
}
...
function rLanc(el){
  ...
  sum.innerHTML='<div class="sbox"><div class="slabel">Receitas</div><div class="sval g">'+fR(rec)+'</div><div class="ssub">'+rc+' lanc.</div></div><div class="sbox"><div class="slabel">Despesas</div><div class="sval r">'+fR(dep)+'</div><div class="ssub">'+dc+' lanc.</div></div>';
  el.appendChild(sum);
  var sb=document.createElement('div');sb.className='search-bar';
  sb.innerHTML='&#128269; ';
  var inp=document.createElement('input');inp.type='text';inp.placeholder='Buscar...';inp.value=buscaQ;inp.addEventListener('input',function(){setBusca(this.value);});
  sb.appendChild(inp);el.appendChild(sb);
  var chips=document.createElement('div');chips.className='chips';
  ['todos','receitas','despesas','fixos','cartao'].forEach(function(f){var c=document.createElement('div');c.className='chip'+(filTx===f?' ativo':'');c.textContent=f.charAt(0).toUpperCase()+f.slice(1);c.addEventListener('click',function(){setFil(f);});chips.appendChild(c);});
...
function rCartoesList(el,d){
  ...
  if(d.cartoes.length===0){var em=document.createElement('div');em.className='card card-pad tx-empty';em.innerHTML='&#x1F4B3;<br>Nenhum cartao adicionado';el.appendChild(em);}
  d.cartoes.forEach(function(c,i){
    ...
    card.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;"><div style="display:flex;align-items:center;gap:12px;"><div style="width:44px;height:44px;border-radius:12px;background:'+b.cor+';display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:800;color:'+b.txt+';">'+b.sigla+'</div><div><div style="font-size:15px;font-weight:600;">'+(c.nome||b.nome)+'</div><div style="font-size:11px;color:var(--text3);">'+(c.bandeira||b.nome)+' &middot; Fecha '+c.diaFecha+'</div></div></div><div style="font-size:11px;font-weight:600;color:'+(diff<=5?'var(--red)':'var(--text3)')+';">'+vencStr+'</div></div>'
    ...
    var bVer=document.createElement('button');bVer.style.cssText='flex:1;padding:10px;background:var(--bg3);border:none;border-radius:var(--rsm);color:var(--text);font-size:13px;font-weight:600;cursor:pointer;';bVer.textContent='Ver detalhes';bVer.onclick=(function(idx){return function(){verCartao(idx);};})(i);
...
function delCC(i){if(!confirm('Excluir este cartao e todos os lancamentos relacionados?'))return;var d=gd();d.cartoes.splice(i,1);save(d);voltarCartoes();toast('Cartao excluido','ok');}
...
function buildGraficoCC(el,c,txs){
  ...
  var isAt=function(md){return md.m===hj2.getMonth()&&md.a===hj2.getFullYear();};
  var isFut=function(md){return md.a>hj2.getFullYear()||(md.a===hj2.getFullYear()&&md.m>hj2.getMonth());};
...
function rRel(el){
  ...
  function mk3(lbl,val,cor,sub){var c=document.createElement('div');c.className='sbox';c.innerHTML='<div class="slabel">'+lbl+'</div><div class="sval '+cor+'">'+fRs(val)+'</div>'+(sub?'<div class="ssub">'+sub+'</div>':'');return c;}
  g3.appendChild(mk3('Entradas',rec,'g',''));
  g3.appendChild(mk3('Saidas',depTotal,'r',''));
  g3.appendChild(mk3('Saldo',saldo,saldo>=0?'g':'r','apos pagos'));
  el.appendChild(g3);
  // Grafico pizza
  ...
  if(td>0){
    ...
    cats.forEach(function(cid){var cat=getCat(cid),val=catMap[cid],ang=(val/td)*2*Math.PI,x1=cx+r*Math.cos(sa),y1=cy+r*Math.sin(sa),x2=cx+r*Math.cos(sa+ang),y2=cy+r*Math.sin(sa+ang),lg=ang>Math.PI?1:0;paths+='<path d="M '+cx+' '+cy+' L '+x1.toFixed(1)+' '+y1.toFixed(1)+' A '+r+' '+r+' 0 '+lg+' 1 '+x2.toFixed(1)+' '+y2.toFixed(1)+' Z" fill="'+cat.cor+'" stroke="#0a0a0a" stroke-width="1.5"/>';sa+=ang;});
    gc.innerHTML='<div style="display:flex;align-items:center;gap:14px;"><svg width="'+svgS+'" height="'+svgS+'" viewBox="0 0 '+svgS+' '+svgS+'" style="flex-shrink:0;">'+paths+'<circle cx="'+cx+'" cy="'+cy+'" r="24" fill="#141414"/></svg><div style="flex:1;">';
    ...
    // Barras por categoria com orcamento
    ...
    var em=document.createElement('div');em.className='card card-pad tx-empty';em.textContent='Sem gastos neste mes';el.appendChild(em);
  }
  // Historico 6 meses
...
function fcM(id){var e=document.getElementById(id);if(e)e.classList.remove('aberto');var abertos=document.querySelectorAll('.sheet.aberto');if(!abertos.length)document.getElementById('overlay-global').style.display='none';}
...
function abTx(tipo,dc){
  ...
  var e;
  ['tx-desc','tx-obs'].forEach(function(id){e=document.getElementById(id);if(e)e.value='';});
  ...
  e=document.getElementById('tx-pt');if(e)e.value='';
  e=document.getElementById('tx-pa');if(e)e.value='';
  ...
  // fechar mais opcoes
  var mc=document.getElementById('mais-opc-content');if(mc)mc.classList.remove('aberto');
  var mi=document.getElementById('mais-opc-ic');if(mi)mi.textContent='+';
...
function abreEditTx(id){
  ...
  // abrir mais opcoes se tiver parcelas ou obs
...
// ---- PAGAMENTOS ----
// pagar hoje com 1 toque (sem modal)
function pagarHoje(id){
  var d=gd(),t=d.transacoes.find(function(x){return x.id===id;});
  if(!t)return;
  if(!t.pagamentos)t.pagamentos={};
  t.pagamentos[ch()]=new Date().toISOString().split('T')[0];
  save(d);toast('Pago!','ok');renderPag();
}

function abrePagTx(id){pagTxId=id;var d=gd(),t=d.transacoes.find(function(x){return x.id===id;});if(!t)return;var e=document.getElementById('pag-info');if(e)e.innerHTML='<div style="font-size:14px;font-weight:600;margin-bottom:4px;">'+t.desc+'</div><div style="font-size:13px;color:var(--red);">'+fR(t.valor)+'</div>';e=document.getElementById('pag-data');if(e)e.value=new Date().toISOString().split('T')[0];abM('sh-pag');}

function confirmarFatura(cartaoId){
  var d=gd(),c=d.cartoes.find(function(x){return x.id===cartaoId;});if(!c)return;
  pagTxId='fat-'+cartaoId;
  var cf=cicloFechado(c),us=usadoCCCiclo(c,d.transacoes,cf);
  var e=document.getElementById('pag-info');
  if(e)e.innerHTML='<div style="font-size:14px;font-weight:600;margin-bottom:4px;">Fatura '+(c.nome||banco(c.banco).nome)+'</div><div style="font-size:13px;color:var(--red);">'+fR(us)+'</div><div style="font-size:11px;color:var(--text3);margin-top:4px;">Vence '+fData(cf.dataVenc)+'</div>';
  e=document.getElementById('pag-data');if(e)e.value=new Date().toISOString().split('T')[0];
...
function confirmaPag(){
  ...
  if(pagTxId&&pagTxId.indexOf('fat-')===0){
    ...
    var atrasado=dataD(data)>dataD(cf.dataVenc);if(atrasado){if(!c.faturasAtraso)c.faturasAtraso={};c.faturasAtraso[chv]=true;}
    save(d);fcM('sh-pag');toast(atrasado?'Pago em atraso':'Fatura paga!',atrasado?'warn':'ok');renderPag();return;
  }
...
function abreAPagar(){
  ...
  var hj=hoje0(),todos=[];
  fps.forEach(function(t){var diff=Math.round((dataD(t.data)-hj)/(864e5));todos.push({tipo:'tx',t:t,diff:diff});});
  fats.forEach(function(f){var diff=Math.round((dataD(f.dataVenc)-hj)/(864e5));todos.push({tipo:'fat',f:f,diff:diff});});
  todos.sort(function(a,b){return a.diff-b.diff;});
  todos.forEach(function(item,idx){
    var diff=item.diff;
    var urgCor=diff<0?'var(--red)':diff===0?'var(--yellow)':'var(--text3)';
    var dataStr=diff<0?'Vencido '+Math.abs(diff)+' dia(s)':diff===0?'Hoje':'Vence '+fData(item.tipo==='fat'?item.f.dataVenc:item.t.data);
    var row=document.createElement('div');row.className='tx-item';
    ...
    var info=document.createElement('div');info.className='tx-info';
    info.innerHTML='<div class="tx-nome">'+nome+'</div><div class="tx-cat" style="color:'+urgCor+'">'+dataStr+'</div>';
    var right=document.createElement('div');right.className='tx-right';
