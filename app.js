...
var editContaId=null,editCartaoId=null;
...
function pushState(estado){history.pushState({fx:estado},'','');}
window.addEventListener('popstate',function(e){
  var ab=document.querySelectorAll('.sheet.aberto');
  if(ab.length>0){ab[ab.length-1].classList.remove('aberto');var resto=document.querySelectorAll('.sheet.aberto');if(!resto.length)document.getElementById('overlay-global').style.display='none';pushState('app');return;}
...
function rInicio(el){
  ...
  // calcular atrasadas
  var hj0=hoje0();
  var txAtrasadas=fpend.filter(function(t){return dataD(t.data)<hj0;});
  var fatAtrasadas=fatsPend.filter(function(f){return dataD(f.dataVenc)<hj0;});
  var totalAtrasadas=txAtrasadas.length+fatAtrasadas.length;
  var valorAtrasado=txAtrasadas.reduce(function(a,t){return a+t.valor;},0)+fatAtrasadas.reduce(function(a,f){return a+f.valor;},0);
  ...
  // ALERTA ATRASADAS
  if(totalAtrasadas>0){
    var alerta=document.createElement('div');alerta.className='alerta-atraso';
    alerta.innerHTML='<div style="font-size:18px;flex-shrink:0;">&#9888;</div><div style="flex:1;"><div style="font-size:13px;font-weight:600;color:var(--red);">'+totalAtrasadas+' conta(s) em atraso</div><div style="font-size:11px;color:var(--red);opacity:.8;margin-top:2px;">'+fR(valorAtrasado)+' vencidos</div></div><div style="font-size:12px;color:var(--red);font-weight:600;">Ver &#8250;</div>';
    alerta.addEventListener('click',function(){abreAPagar();});
  ...
  var mEnt=document.createElement('div');mEnt.className='metrica';mEnt.innerHTML='<div class="metrica-label">Entradas</div><div class="metrica-val" style="color:var(--accent);">'+(privado?'R$ ****':fR(rec))+'</div>';mEnt.addEventListener('click',function(){abrePagos();});
  var mSai=document.createElement('div');mSai.className='metrica';mSai.innerHTML='<div class="metrica-label">Saidas</div><div class="metrica-val" style="color:var(--red);">'+(privado?'R$ ****':fR(depPagoTotal))+'</div>';mSai.addEventListener('click',function(){abrePagos();});
  ...
  if(d.contas.length===0){var noC=document.createElement('div');noC.className='ac-card add';noC.style.minWidth='160px';noC.textContent='+ Adicionar conta';noC.addEventListener('click',function(){abreNovaConta();});hs.appendChild(noC);}
  ...
  if(d.cartoes.length>0){
    ...
    d.cartoes.forEach(function(c,i){
      ...
      var cd=document.createElement('div');cd.className='ac-card';cd.style.cssText='min-width:160px;background:var(--bg2);border-radius:14px;padding:12px 14px;'+(fatPend2?'border:.5px solid rgba(248,113,113,.4);':'');
...
function rCartaoDetalhe(el,d){
  ...
  hdr.appendChild(bk);
  var hInfo=document.createElement('div');hInfo.style.cssText='flex:1;';hInfo.innerHTML='<div style="display:flex;align-items:center;gap:10px;"><div style="width:42px;height:42px;border-radius:12px;background:'+b.cor+';display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:'+b.txt+';">'+b.sigla+'</div><div><div style="font-size:16px;font-weight:700;">'+(c.nome||b.nome)+'</div><div style="font-size:11px;color:var(--text3);">'+(c.bandeira||b.nome)+' &middot; Fecha '+c.diaFecha+' &middot; Vence '+c.diaVence+'</div></div></div>';
  hdr.appendChild(hInfo);
  var bEdit=document.createElement('button');bEdit.style.cssText='background:var(--bg3);border:none;border-radius:8px;padding:6px 10px;font-size:12px;color:var(--text2);cursor:pointer;flex-shrink:0;';bEdit.textContent='Editar';bEdit.onclick=function(){abreEditCartao(c.id);};hdr.appendChild(bEdit);
  el.appendChild(hdr);
  ...
  btns.appendChild(bl);el.appendChild(btns);
...
function rRel(el){
  ...
  // VISAO GERAL
  var t1=document.createElement('div');t1.className='rel-sec';t1.textContent='Visao Geral';el.appendChild(t1);
  ...
  if(td>0){
    ...
    var emCat=document.createElement('div');emCat.className='card card-pad tx-empty';emCat.textContent='Nenhum gasto lancado neste mes';el.appendChild(emCat);
  }
  ...
  // METAS
  var t4=document.createElement('div');t4.className='rel-sec';t4.style.display='flex';t4.style.justifyContent='space-between';t4.style.alignItems='center';
  t4.innerHTML='<span>Metas</span>';
  var mAdd=document.createElement('span');mAdd.style.cssText='color:var(--accent);cursor:pointer;font-size:11px;font-weight:500;';mAdd.textContent='+ Nova';mAdd.addEventListener('click',function(){abM('sh-meta');});t4.appendChild(mAdd);el.appendChild(t4);
  if(d.metas.length>0){
    ...
    var emMeta=document.createElement('div');emMeta.className='card card-pad tx-empty';emMeta.textContent='Nenhuma meta criada ainda';el.appendChild(emMeta);
  }

  // ORCAMENTOS
  var t5=document.createElement('div');t5.className='rel-sec';t5.style.display='flex';t5.style.justifyContent='space-between';t5.style.alignItems='center';
  t5.innerHTML='<span>Orcamentos</span>';
  var oAdd=document.createElement('span');oAdd.style.cssText='color:var(--accent);cursor:pointer;font-size:11px;font-weight:500;';oAdd.textContent='+ Definir';oAdd.addEventListener('click',function(){abM('sh-orc');});t5.appendChild(oAdd);el.appendChild(t5);
  var orcKeys=Object.keys(d.orcamentos||{});
  if(orcKeys.length>0){
    var oCard=document.createElement('div');oCard.className='card card-pad';
    orcKeys.forEach(function(cid){var cat=getCat(cid),lim=d.orcamentos[cid],gasto=catMap[cid]||0,pct=lim>0?Math.min(100,(gasto/lim)*100):0,bc=pct>90?'var(--red)':pct>70?'var(--yellow)':'var(--accent)';oCard.innerHTML+='<div class="bud-item"><div class="bud-hdr"><div class="bud-cat"><span>'+cat.ic+'</span>'+cat.nome+'</div><div class="bud-vals">'+fR(gasto)+' / '+fR(lim)+'</div></div><div class="bud-bar"><div class="bud-fill" style="width:'+pct+'%;background:'+bc+'"></div></div><div class="bud-pct" style="color:'+bc+'">'+pct.toFixed(0)+'% usado</div></div>';});
    el.appendChild(oCard);
  } else {
    var emOrc=document.createElement('div');emOrc.className='card card-pad tx-empty';emOrc.textContent='Nenhum orcamento definido';el.appendChild(emOrc);
  }

  // CARTOES
  if(d.cartoes.length>0){
    var t6=document.createElement('div');t6.className='rel-sec';t6.textContent='Cartoes';el.appendChild(t6);
    ...
    var t6b=document.createElement('div');t6b.className='rel-sec';t6b.textContent='Cartoes';el.appendChild(t6b);
    var emK=document.createElement('div');emK.className='card card-pad tx-empty';emK.textContent='Nenhum cartao cadastrado';el.appendChild(emK);
  }
}
...
function deletaCartaoEdit(){if(!editCartaoId)return;if(!confirm('Excluir este cartao?'))return;var d=gd();d.cartoes=d.cartoes.filter(function(c){return c.id!==editCartaoId;});save(d);fcM('sh-cartao');editCartaoId=null;toast('Cartao excluido!','ok');renderPag();}
